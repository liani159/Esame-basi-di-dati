...
INSERT INTO NOLLEGGIA(MATRICOLA, CODICE_UNIVOCO, ISBN_ID, ID_BIBLIOTECA,
	...
	SELECT (SELECT MATRICOLA FROM STUDENTE
		...
		(C.CODICE_UNIVOCO-1) CODICE_UNIVOCO, C.ISBN_ID ISBN_ID,
		C.ID_BIBLIOTECA ID_BIBLIOTECA, curdate(), curdate()+interval 5 day
	FROM COPIA C
	LEFT JOIN LIBRI L
		ON C.ISBN_ID=L.ISBN_ID
	WHERE L.TITOLO="Enemy"
...
SELECT S.NOME , S.COGNOME, S.MATRICOLA, L.TITOLO, N.CODICE_UNIVOCO CU_LIBRO,
		N.DATA_USCITA, N.DATA_RITORNO, B.NOME_BIBLIOTECA, B.ID_BIBLIOTECA ID_BIB
		FROM NOLLEGGIA N
		LEFT JOIN  STUDENTE S
			ON S.MATRICOLA= N.MATRICOLA
		LEFT JOIN LIBRI L
...
-- Test reinserimento della copia nella sua biblioteca dopo cancellazzione
-- di un Nolleggio

-- JUST TO RETREIVE CODICE UNIVOCO IN COPIA
SELECT  C.CODICE_UNIVOCO CODICE_UNIVOCO
	FROM COPIA C
	LEFT JOIN LIBRI L
		ON C.ISBN_ID=L.ISBN_ID
	WHERE L.TITOLO="Enemy"
...
SELECT  C.CODICE_UNIVOCO CODICE_UNIVOCO, C.ISBN_ID ISBN_ID,
	C.ID_BIBLIOTECA ID_BIBLIOTECA
	FROM COPIA C
	LEFT JOIN LIBRI L
		ON C.ISBN_ID=L.ISBN_ID
	WHERE L.TITOLO="Enemy"
...
-- Query complementari
-- Studente che ha fatto piu nolleggi

SELECT MATRICOLA,
	COUNT(MATRICOLA) AS `OCCORENZA` 
	FROM NOLLEGGIA
    GROUP BY MATRICOLA
